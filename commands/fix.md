...
8. When Andy completes implementation, updates task status to `ready for review`
9. Wait for wave completion before starting dependent tasks
10. Track progress across all Andy instances

### Phase 3: Code Review
11. Invoke **Rick** to review tasks marked as `ready for review`
    - Rick reads the specific task file(s) for context
12. Rick updates task status to `reviewing` in the task file
13. If changes requested: loop back to Andy for fixes (task stays `reviewing`)
14. If approved: Rick updates task status to `completed` in the task file
15. Save review notes to `review-notes.md`

### Phase 4: Verification & Ship
16. Ask human to verify the bug fix:
...
17. Based on human feedback:
    - **If code changes required**: Start the fix loop (go to step 18)
    - **If only documentation/follow-ups needed**: Document them for future work (go to step 22)
    - **If verified complete**: Mark the fix as ready to ship (go to step 22)

18. **Fix Loop - Problem Identification**:
...
20. **Fix Loop - Code Review**:
    - Invoke **Rick** to review the fixes
    - Rick reads task file(s) and updates status to `reviewing`
    - If changes requested: loop back to Andy (step 19)
    - If approved: Rick updates task status to `completed`

21. **Fix Loop - Re-verification**:
    - Return to step 16 (human verification)
    - Loop continues until human confirms no more code changes needed

22. Document any remaining follow-up tasks for future work
23. Mark the fix as ready to ship
...
- The verification loop (Phase 4, steps 16-21) ensures human satisfaction
